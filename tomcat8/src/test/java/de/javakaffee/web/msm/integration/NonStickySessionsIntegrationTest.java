...
public abstract class NonStickySessionsIntegrationTest {
    ...
    private Tomcat _tomcat1;
    private Tomcat _tomcat2;
    ...
    private Tomcat startTomcat( final int port ) throws MalformedURLException, UnknownHostException, LifecycleException {
        return startTomcat(port, MEMCACHED_NODES, null);
    }

    private Tomcat startTomcat( final int port, final String memcachedNodes, final LockingMode lockingMode ) throws MalformedURLException, UnknownHostException, LifecycleException {
        final Tomcat tomcat = getTestUtils().tomcatBuilder().port(port).sessionTimeout(5).memcachedNodes(memcachedNodes)
    ...
    private Tomcat startTomcatWithAuth( final int port, @Nonnull final LockingMode lockingMode ) throws MalformedURLException, UnknownHostException, LifecycleException {
        return startTomcatWithAuth(port, MEMCACHED_NODES, lockingMode, LoginType.BASIC);
    }

    private Tomcat startTomcatWithAuth(final int port, final String memcachedNodes, final LockingMode lockingMode, final LoginType loginType)
            throws MalformedURLException, UnknownHostException, LifecycleException {
        final Tomcat result = getTestUtils().tomcatBuilder().port(port).sessionTimeout(5).loginType(loginType)
