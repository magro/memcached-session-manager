...
public abstract class TomcatFailoverIntegrationTest {
    ...
    private Tomcat startTomcat( final int port, final String jvmRoute ) throws MalformedURLException, UnknownHostException, LifecycleException {
        return startTomcat( port, SessionAffinityMode.STICKY, jvmRoute, null );
    }

    private Tomcat startTomcat( final int port, final SessionAffinityMode sessionAffinityMode,
                                  final String jvmRoute, final LoginType loginType ) throws MalformedURLException, UnknownHostException, LifecycleException {
        final Tomcat tomcat = getTestUtils().tomcatBuilder().port(port).memcachedNodes(_memcachedNodes)
