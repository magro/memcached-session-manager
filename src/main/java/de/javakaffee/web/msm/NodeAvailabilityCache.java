...
public class NodeAvailabilityCache<K> {
     ...
     * @param cacheLoader 
     */
    public NodeAvailabilityCache(int size, long ttlInMillis, final CacheLoader<K> cacheLoader) {
        _ttl = ttlInMillis;
     ...
     * @param available the value to associate with the provided key.
    ...
    public Boolean setNodeAvailable( K key, boolean available ) {
        ...
        if ( item == null || item.value != availableObj ) {
            final ManagedItem<Boolean> previous = _map.put( key, new ManagedItem<Boolean>( availableObj, System.currentTimeMillis() ) );
    ...
    public boolean isNodeAvailable( K key ) {
        final ManagedItem<Boolean> item = _map.get( key );
        if ( item == null ) {
    ...
    public Set<K> getUnavailableNodes() {
        ...
        for ( Map.Entry<K, ManagedItem<Boolean>> entry : _map.entrySet() ) {
